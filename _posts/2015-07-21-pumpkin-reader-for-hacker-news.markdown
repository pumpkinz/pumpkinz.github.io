...
Also, there are now a lot of open source libraries we can use to help us in Android development. For Pumpkin Reader, we used [Retrofit][retrofit], combined with [RxJava][rxjava] and [RxAndroid][rxandroid]. Harnessing the power of those open source softwares really helped us to get the foundation done sooner, therefore we can focus on our main features faster.
